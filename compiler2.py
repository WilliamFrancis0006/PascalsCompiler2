...
####################################################################
        ...
        if sym == "ELSE":
            ...
            statement(tx, level)      
            fixJmp(cx2, codeIndx)    # fix JMP @ c2
...
######################################################################################
        ...
        while sym == "comma":
            getsym()
            expression(tx, level)
            gen("OPR", 0, 14)       # OPR 0, 12
        if sym != "rparen":
            error(22) 
        getsym()
        if (writeSum == "WRITELN"): # if sym is wrtieln:
...
###########################################################################################
        ...
        getsym()
        expression(tx, level)
        gen("STO", level -table[i].level, table[i].adr)     # STO level -table[i].level, table[i].adr
        ...
        gen("CTS", 0, 0)                                    # CTS 0,0
        gen("LOD", level-table[i].level, table[i].adr)      # LOD level -table[i].level, table[i].adr
        if ToDown == "TO":                                  # if sym is TO
            gen("OPR", 0, 11)                                   # OPR 0, >= (Might have to change)
        elif ToDown == "DOWNTO":                            # if sym is DOWNTO
            gen("OPR", 0, 13)                                   # OPR 0, <= (Might have to change)
        else: 
            error(1) # just in case?
        cx2 = codeIndx                                      # save cx2
        gen("JPC", 0, 0)                                    # JPC 0, 0
        if sym != "DO":
            error(18)
        ...
        gen("LIT", 0, 1)                                    # LIT 0,1
        if ToDown == "TO":                                  # if sym is TO
            gen("OPR", 0, 2)                                   # OPR 0, + (Might have to change)
        elif ToDown == "DOWNTO":                            # if sym is DOWNTO
            gen("OPR", 0, 3)                                   # OPR 0, - (Might have to change)
        else: 
            error(1) # just in case?
        gen("STO", level-table[i].level, table[i].adr)     # STO level -table[i].level, table[i].adr
